...
1. **権限の最小化**: claude-code-actionを使うworkflowには、原則として
   ...
   - **例外1**: `.github/workflows/dependabot-auto-merge.yml`（#66、
     #177で判定方式を変更）は`contents: write`（マージ権限）を持つが、
     claude-code-actionは使用しない。PR作成者が`dependabot[bot]`で
     あること・draftでないこと・CIの全チェック結果という、LLMの出力を
     一切介さない機械的な条件のみでマージ可否を判定する。最終的な
     マージアクション自体は人間が読める条件分岐に委ねている。
...
2. **ラベル方式の間接的な自動化**: マージ・適用などの最終アクションが
   必要な場合、Claudeは判定結果を`needs-human-review`等のラベル付与や
...
7. **CLAUDE_CODE_OAUTH_TOKENの運用方針**: `ANTHROPIC_API_KEY`の代わりに、
   ...
   - **`dependabot-review.yml`は廃止した**（#66で追加、#177で廃止）:
     Dependabot作成のPRに対する`pull_request`イベントも、fork PRと
     同様にGitHubが制限付きコンテキストとして扱い、`GITHUB_TOKEN`以外の
     secretsが渡らない。上のfork PRのケースは「意図した安全側no-op」
     だったが、こちらは見落としによるバグで、#131以降このworkflowの
     レビュー・自動マージが実質的に一度も機能していなかった
     （#173〜#176は結局手動マージが必要になった）。`pull_request_target`
     に切り替えれば技術的には修正できたが、kamegoroの判断で
     「Claudeによる内容判定」自体を廃止し、`dependabot-auto-merge.yml`が
     PR作成者・draft・CI結果のみで判定する方式に一本化した（決定事項1
     例外1を参照）。これによりメジャーバージョンアップも含めてCIが
     全passすれば自動マージされる。バージョンの種類やリリースノートの
     内容による選別は行わない。
...
- Dependabot PRのレビュー（#66）はPR本文やdiffに埋め込まれた
  プロンプトインジェクションのリスクがあった上、#177で判明した
  secrets制限により実質的に機能していなかった。kamegoroの判断により
  この分類ステップ自体を廃止したため、このリスクは無くなった一方、
  `dependabot-auto-merge.yml`はバージョンの種類（メジャー/マイナー/
  パッチ）やリリースノートの内容を判定せず、CIが全passすれば
  自動マージする。CIが検知できない破壊的変更（依存関係の互換性問題等）は
  自動マージ後に発覚し得るというトレードオフを許容している
...
- ラベル体系（`.github/labels.yml`）の`needs-human-review`等を、
  この方針に基づいて運用する
...
- `dependabot-review.yml`（#66でClaudeによる分類を追加）は#177で廃止した。
  Dependabot作成のPRに対する`pull_request`イベントはsecretsが渡らず
  実質的に機能していなかったため
- `dependabot-auto-merge.yml`は決定事項1の例外として`contents: write`を
  持つが、claude-code-actionは使用せず、PR作成者・draft・CI結果のみで
  マージ可否を判定する（#66、#177で判定方式を変更）。トリガーは
  `workflow_run`（Terraform CI完了時）のみとし、Dependabot作成のPRに
  対する`pull_request`イベントの権限制限を回避する
