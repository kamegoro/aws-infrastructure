...
7. **CLAUDE_CODE_OAUTH_TOKENの運用方針**: `ANTHROPIC_API_KEY`の代わりに、
   ...
   - **`dependabot-review.yml`は`pull_request_target`を使う**（#177）:
     Dependabot作成のPRに対する`pull_request`イベントも、fork PRと
     同様にGitHubが制限付きコンテキストとして扱い、`GITHUB_TOKEN`以外の
     secretsが渡らない。上のfork PRのケースは「意図した安全側no-op」だが、
     こちらは見落としによるバグで、#131以降このworkflowのレビュー・
     自動マージが実質的に一度も機能していなかった（#173〜#176は結局
     手動マージが必要になった）。`pull_request_target`はbase repo
     コンテキストで実行されsecretsにアクセスできる。デフォルトの
     checkoutはPRのhead refではなくbase refのため、Dependabot PRの
     内容（コード）がそのまま実行されることはなく、Claude側のツールも
     `gh pr view`/`gh pr diff`の読み取りのみに限定したままなので、
     上の「理由」節で説明したプロンプトインジェクション対策の脅威モデルは
     変わらない。
