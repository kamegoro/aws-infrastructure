...
- 変更はGitHub Actions（`.github/workflows/terraform.yml`）の
  `terraform fmt -check` / `tflint` / `tfsec` / `terraform validate` /
  MiniStackでの`envs/local`全体の`plan`・`apply`/`destroy`がすべてpassすることを
  確認する
...
- 複数セッションに渡りそうな大きめのissueに着手する際は、
  `docs/claude-progress/issue-<番号>.md`を作成し、対応中のissue/ブランチ、
  完了したステップ、残作業、懸念事項を記録する
  （[docs/claude-progress/README.md](docs/claude-progress/README.md)参照）
- 作業の区切りで進捗ファイルを更新し、issueがクローズ・PRがマージされたら
  削除する（重要な知見は`docs/adr/`に昇華させる）
...
- `needs-human-review` / `reviewed-by-claude` / `claude-ready`
...
- 新しいモジュールを追加したら `versions.tf` に `required_version` /
  `required_providers` を定義する（[.tflint.hcl](.tflint.hcl)で
  `terraform_required_providers` / `terraform_required_version` を
  チェックしている）
- セキュリティ上の懸念は`tfsec`（[.tfsec/config.yml](.tfsec/config.yml)）で
  チェックする。新たな検出項目が出た場合は、対応するか除外するかを
  [ADR 0005](docs/adr/0005-tfsec-exclusions.md)の方針に基づいて判断し、
  除外する場合は理由をADRに追記する

## MiniStackについて

- [MiniStack](https://github.com/ministackorg/ministack)（無料、サインアップ不要）を前提とする
- `network` / `static-site` / `fargate-service` / `database` / `secrets` / `ecr` すべてのモジュールでapply/destroyまで確認可能
